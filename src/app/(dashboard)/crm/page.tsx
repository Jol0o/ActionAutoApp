...
export default function CrmLoginPage() {
  ...
  const handleBiometricLogin = async () => {
    ...
    try {
      ...
      //    Optionally pass username to restrict to that user's credentials
      const optRes = await apiClient.post("/api/crm/biometric/auth/options", {
        username: username.trim() || undefined,
      })
      const { options, storeKey } = optRes.data?.data

      // 2. Trigger biometric prompt via browser WebAuthn API
      ...
      // Don't show error if user simply cancelled the prompt
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-10 bg-background">
      <div className="w-full max-w-[400px] space-y-8">
          ...
          {biometricAvailable && (
            <>
              ...
              >
                ...
                {biometricLoading ? "Verifying biometric…" : "Sign in with Biometrics"}
              </Button>
        ...
        <p className="text-center text-[10px] text-muted-foreground/30">
          ...
          {biometricAvailable && " · Biometric MFA enabled"}
        </p>
      </div>
    </div>
  )
