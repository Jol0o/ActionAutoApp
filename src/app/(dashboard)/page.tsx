...
export default function Dashboard() {
  ...
  const kpiData = [
    {
      ...
      value: isLoading
        ? "..."
        : metrics?.stats?.activeInventory?.toString() || "0",
    ...
    },
    {
      ...
      value: isLoading
        ? "..."
        : metrics?.stats?.monthlyQuotes?.toString() || "0",
    ...
    },
    {
      ...
      value: isLoading
        ? "..."
        : formatCurrency(metrics?.stats?.potentialRevenue || 0),
    ...
    },
    {
      ...
      value: isLoading
        ? "..."
        : metrics?.stats?.avgDaysOnLot?.toString() || "14",
  ...
  const logisticsData = metrics?.logistics || {
    drivers: { active: 0, ready: 0 },
    shipments: {},
  };
    ...
    <div className="p-4 sm:p-8 space-y-8 container mx-auto min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <Badge
              variant="outline"
              className="bg-primary/5 text-primary border-primary/20 text-[10px] font-bold tracking-widest uppercase px-2 py-0.5"
            >
              Management Portal
            </Badge>
        ...
        <div className="flex items-center gap-3">
          <div className="flex -space-x-2 mr-2">
            {leaderboardData.slice(0, 4).map((rep: any) => (
              <Avatar
                key={rep.name}
                className="border-2 border-background lg:size-10 md:size-8 size-6 shadow-sm transition-transform hover:scale-110 cursor-pointer"
              >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {kpiData.map((kpi) => (
          <Card
            key={kpi.label}
            className="p-0 border-border/50 shadow-sm hover:shadow-xl transition-all duration-300 group relative overflow-hidden bg-card/80 backdrop-blur-sm border"
          >
            ...
            <CardContent className="p-6 flex justify-between">
              ...
              <div className="flex items-start justify-between mb-4">
                <Badge
                  variant="secondary"
                  className={`text-[10px] font-black ${kpi.trendUp ? "bg-emerald-500/10 text-emerald-500" : "bg-rose-500/10 text-rose-500"}`}
                >
                  {kpi.trend}
                </Badge>
              </div>
        ...
        <Card className="lg:col-span-8 p-0 border-border/50 bg-card overflow-hidden shadow-sm gap-0">
          <CardHeader className="py-5 px-8 border-b border-border/10 flex flex-row items-center justify-between space-y-0">
            <div>
              ...
              <CardDescription className="text-xs uppercase font-bold tracking-tight text-muted-foreground/60">
                Comprehensive dealership member metrics
              </CardDescription>
            </div>
              ...
              <Table>
                <TableHeader className="bg-muted/30">
                  <TableRow className="border-border/50">
                    ...
                    <TableHead className="py-3 text-[10px] font-black uppercase tracking-widest text-center">
                      Calls
                    </TableHead>
                    <TableHead className="py-3 text-[10px] font-black uppercase tracking-widest text-center">
                      Convs
                ...
                <TableBody>
                  {leaderboardData.map((rep: any, idx: number) => (
                    <TableRow
                      key={rep.name}
                      className="hover:bg-muted/20 border-border/50 transition-colors h-16"
                    >
                        ...
                        <div className="flex items-center gap-3">
                          <div className="relative">
                            <Avatar className="size-9 border border-border">
                              ...
                              <AvatarFallback className="font-bold text-xs bg-muted uppercase">
                                {rep.name[0]}
                              </AvatarFallback>
                            </Avatar>
                            <div
                              className={`absolute -top-1 -right-1 size-4 rounded-full flex items-center justify-center text-[8px] font-black text-white ${idx === 0 ? "bg-primary" : "bg-muted-foreground/40"}`}
                            >
                              {idx + 1}
                            </div>
                          </div>
                      ...
                      <TableCell className=" text-center font-black text-sm text-emerald-500">
                        {rep.shipments}
                      </TableCell>
                    </TableRow>
                  ))}
        ...
        <div className="lg:col-span-4 space-y-6">
          <Card className="border-border/50 p-0 bg-card shadow-sm gap-0">
            ...
            <CardContent className="p-3 grid grid-cols-2 gap-2">
              <div className="p-3 bg-emerald-500/5 rounded-2xl border border-emerald-500/10 text-center">
                <p className="text-3xl font-black text-emerald-500 leading-none mb-1">
          ...
          <Card className="border-border/50 p-0 bg-card shadow-sm flex-1 gap-0">
            ...
            <CardContent className="p-4 space-y-2.5">
              ...
              <Link
                href="/transportation"
                className="pt-3 border-t border-border/50 flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-primary hover:underline cursor-pointer group"
              >
                View Full Pipeline
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="space-y-3">
                ...
                <p className="text-[10px] font-bold text-muted-foreground uppercase">
                  Target: 14 Days | Current:{" "}
                  {metrics?.stats?.avgDaysOnLot || 14} Days
                </p>
              </div>
              <div className="space-y-3">
                <h4 className="text-lg font-black tracking-tighter">
