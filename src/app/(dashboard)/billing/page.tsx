...
import {
  ...
  CheckCircle2, XCircle, Activity, CreditCard, AlertCircle,
  Wallet, Banknote, Shield, Eye, EyeOff, ArrowRight,
...
// ─── Design Tokens ─────────────────────────────────────────────────────────────
// Surface/text/border stay on your existing CSS vars.
// SuprahPay brand is emerald green — injected as fixed values.
const T = {
  ...
  return (
    <span style={{
      ...
      <span style={{
        ...
        textTransform: "uppercase", color: T.brandMid,
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span style={{
      ...
      color, background: bg, padding: "2px 8px", borderRadius: 6,
...
// ─── Stat Card ─────────────────────────────────────────────────────────────────
function StatCard({
  icon: Icon, label, value, sub, colorVar, bgVar, glowColor, delay = 0, skeleton,
}: {
  icon: React.ElementType; label: string; value: string; sub: string;
  colorVar: string; bgVar: string; glowColor?: string; delay?: number; skeleton?: boolean;
}) {
  return (
    <div
      className="sp-stat"
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 18 }}>
        <div style={{
          width: 38, height: 38, borderRadius: 11, background: bgVar,
        ...
        <span style={{
          ...
          textTransform: "uppercase", fontFamily: "var(--font-mono)", color: T.textMute,
        }}>{label}</span>
      </div>
      ...
      {skeleton
        ...
        : <p style={{
            fontSize: 28, fontWeight: 700, margin: 0, color: T.text,
            letterSpacing: "-0.03em", lineHeight: 1,
            fontFamily: "'Epilogue', var(--font-mono), monospace",
          }}>{value}</p>
      }
      <p style={{
        fontSize: 11, color: T.textMute, margin: "8px 0 0",
...
// ─── Nav Card ──────────────────────────────────────────────────────────────────
function NavCard({
  href, icon: Icon, title, desc, badge, colorVar, bgVar, delay = 0,
}: {
  href: string; icon: React.ElementType; title: string; desc: string;
  badge?: number; colorVar: string; bgVar: string; delay?: number;
}) {
  return (
    <Link
      href={href}
    ...
    >
      <div style={{
        ...
        background: `linear-gradient(90deg, ${colorVar}, transparent 60%)`, opacity: 0.4,
      }} />

      <div style={{
        width: 42, height: 42, borderRadius: 12, background: bgVar, flexShrink: 0,
        display: "flex", alignItems: "center", justifyContent: "center",
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontSize: 14, fontWeight: 600, color: T.text, margin: 0, letterSpacing: "-0.01em" }}>{title}</p>
        <p style={{ fontSize: 12, color: T.textSub, margin: "2px 0 0", lineHeight: 1.45 }}>{desc}</p>
      </div>

      <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
        {badge !== undefined && badge > 0 && (
          <span style={{
            fontSize: 10, fontWeight: 700, background: T.warningBg, color: T.warning,
            borderRadius: 6, padding: "1px 8px", fontFamily: "var(--font-mono)",
            border: `1px solid ${T.border}`,
          }}>{badge}</span>
        )}
        <div style={{
...
// ─── Transaction Row ───────────────────────────────────────────────────────────
function TxRow({ payment, idx }: { payment: Payment; idx: number }) {
  return (
    <div
      className="sp-tx"
      style={{
    ...
    >
      <div style={{
        ...
        background: T.brandLight, flexShrink: 0,
        display: "flex", alignItems: "center", justifyContent: "center",
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        }}>{payment.customerName}</p>
        <p style={{
          fontSize: 11, color: T.textMute, margin: "2px 0 0",
          fontFamily: "var(--font-mono)",
          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
        }}>{payment.invoiceNumber ?? payment.description}</p>
      </div>

      <div style={{ textAlign: "right", flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 5 }}>
        <p style={{
          fontSize: 14, fontWeight: 700, color: T.text, margin: 0,
          letterSpacing: "-0.02em", fontFamily: "'Epilogue', monospace",
...
// ─── Mini Metric ──────────────────────────────────────────────────────────────
function MiniMetric({ label, value, icon: Icon, colorVar, bgVar, skeleton }: {
  label: string; value: string; icon: React.ElementType;
  colorVar: string; bgVar: string; skeleton?: boolean;
}) {
  return (
    <div style={{
      background: T.surface, border: `1px solid ${T.border}`,
      ...
      <div style={{
        width: 36, height: 36, borderRadius: 10, background: bgVar,
        display: "flex", alignItems: "center", justifyContent: "center",
        border: `1px solid ${T.border}`, flexShrink: 0,
      }}>
      ...
      <div style={{ minWidth: 0 }}>
        <p style={{
          fontSize: 9, fontWeight: 700, color: T.textMute,
          margin: 0, letterSpacing: "0.12em", textTransform: "uppercase",
          fontFamily: "var(--font-mono)",
        }}>{label}</p>
        {skeleton
          ? <div style={{ marginTop: 4 }}><Skeleton w={56} h={18} r={4} /></div>
          : <p style={{
...
export default function BillingDashboard() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const [paymentsRes, statsRes, balanceRes, pendingRes] = await Promise.allSettled([
      ...
      if (paymentsRes.status === "fulfilled") setRecentPayments(paymentsRes.value.data.data.payments?.slice(0, 6) ?? []);
      ...
      if (pendingRes.status === "fulfilled") setPendingCount((pendingRes.value.data.data ?? []).length);
  ...
  const winRate   = succeeded?.count && stats?.totalCount
    ? Math.round(succeeded.count / stats.totalCount * 100) : 0;
  const avgDeal   = (stats?.totalRevenue ?? 0) / Math.max(stats?.totalCount ?? 1, 1);
    ...
    <>
      <style>{`
        ...
        @media (max-width: 600px) {
          ...
          .sp-header  { flex-direction: column !important; align-items: flex-start !important; gap: 12px !important; }
        ...
        <div style={{ maxWidth: 1120, margin: "0 auto", padding: "28px 20px 80px" }}>
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div>
                <h1 style={{
                  ...
                  letterSpacing: "-0.025em", fontFamily: "'Epilogue', sans-serif",
                }}>
                  Suprah<span style={{ color: T.brandMid }}>Pay</span>
                </h1>
                <p style={{
                  fontSize: 10, color: T.textMute, margin: "1px 0 0",
                  fontFamily: "var(--font-mono)", letterSpacing: "0.1em",
                  textTransform: "uppercase", fontWeight: 600,
                }}>Payment Operations</p>
              </div>
            </div>
            ...
            <Link
              ...
              style={{
                ...
                color: T.brandMid, textDecoration: "none",
          ...
          >
            ...
            <div style={{ animation: "fadeUp 0.42s ease 0.06s both" }}>
              ...
              >
                ...
                <div style={{
                  ...
                  backgroundSize: "38px 38px", opacity: 0.22, pointerEvents: "none",
                }} />
                ...
                <div style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{
                      ...
                      <span style={{
                        fontSize: 9, fontWeight: 700, color: T.brandMid,
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: 9, cursor: "pointer", color: T.textSub,
                        padding: "5px 7px", display: "flex", alignItems: "center",
                        transition: "all 0.14s",
                      }}
                    >
                      {isHidden ? <EyeOff style={{ width: 13, height: 13 }} /> : <Eye style={{ width: 13, height: 13 }} />}
                    </button>
                  </div>
                </div>
                ...
                <div style={{ marginBottom: 28, position: "relative" }}>
                  {isLoading
                    ...
                    : <p
                        ...
                        style={{
                          fontSize: 58, fontWeight: 800, color: T.text,
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 10 }}>
                    ...
                    }}>{userId} · SuprahPay Account</p>
                  </div>
                </div>

                {/* Brand divider */}
                <div style={{
                  height: 1, marginBottom: 26, position: "relative",
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", position: "relative" }}>
                  ...
                  ] as const).map(({ label, icon: Icon, cb, primary }) => (
                    <button
                      key={label}
                      onClick={cb}
                      ...
                      style={{
                        ...
                        ...(primary ? {
                          background: T.brand, border: "1px solid transparent", color: "#fff",
              ...
              >
                <MiniMetric
                  label="Win Rate" icon={TrendingUp}
                  value={isLoading ? "" : `${winRate}%`} skeleton={isLoading}
                  colorVar={T.brandMid} bgVar={T.brandLight}
                />
                <MiniMetric
                  label="Avg Deal" icon={BarChart3}
                  value={isLoading ? "" : formatCurrency(avgDeal)} skeleton={isLoading}
                  colorVar={T.accent} bgVar={T.accentBg}
                />
                <MiniMetric
                  label="Volume" icon={Zap}
                  value={isLoading ? "" : String(stats?.totalCount ?? 0)} skeleton={isLoading}
                  colorVar={T.warning} bgVar={T.warningBg}
                />
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <Link href="/billing/payments" style={{
                  display: "flex", alignItems: "center", gap: 4,
                  fontSize: 11, fontWeight: 600, color: T.brandMid,
                ...
                {isLoading
                  ? Array.from({ length: 5 }).map((_, i) => (
                      <div key={i} style={{
                        ...
                        <div style={{ flex: 1 }}>
                          <Skeleton w="52%" h={12} />
                          <div style={{ marginTop: 6 }}><Skeleton w="30%" h={10} /></div>
                        </div>
                  ...
                  : recentPayments.length === 0
                    ? <div style={{ padding: "60px 0", textAlign: "center" }}>
                        <Wallet style={{ width: 30, height: 30, color: T.textMute, margin: "0 auto 12px", display: "block" }} />
                        <p style={{ fontSize: 14, color: T.textSub, fontWeight: 600, margin: 0 }}>No transactions yet</p>
                        <p style={{ fontSize: 12, color: T.textMute, margin: "4px 0 0" }}>Payments will appear here</p>
                      </div>
                    : recentPayments.map((p, i) => <TxRow key={p._id} payment={p} idx={i} />)
                }
              </div>
            </div>
          </div>
          ...
          <section style={{ marginBottom: 20, animation: "fadeUp 0.42s ease 0.16s both" }}>
            <p style={{
              ...
              fontFamily: "var(--font-mono)", margin: "0 0 14px",
            }}>Overview</p>
            <div
              className="sp-stats"
            ...
            >
              <StatCard
                icon={CheckCircle2} label="Revenue" skeleton={isLoading}
                value={isHidden ? "₱ ••••" : formatCurrency(stats?.totalRevenue ?? 0)}
                sub={`${succeeded?.count ?? 0} deals closed`}
                colorVar={T.brandMid} bgVar={T.brandLight} delay={60}
              />
              <StatCard
                icon={Zap} label="Pending" skeleton={isLoading}
                value={isHidden ? "₱ ••••" : formatCurrency(pending?.totalAmount ?? 0)}
                sub={`${pending?.count ?? 0} awaiting`}
                colorVar={T.warning} bgVar={T.warningBg} delay={110}
              />
              <StatCard
                icon={XCircle} label="Failed" skeleton={isLoading}
                value={isHidden ? "₱ ••••" : formatCurrency(failed?.totalAmount ?? 0)}
                sub={`${failed?.count ?? 0} failed`}
                colorVar={T.danger} bgVar={T.dangerBg} delay={160}
              />
              <StatCard
                icon={BarChart3} label="All Time" skeleton={isLoading}
                value={String(stats?.totalCount ?? 0)}
                sub="total payments"
                colorVar={T.accent} bgVar={T.accentBg} delay={210}
              />
            </div>
          </section>
          ...
          <section style={{ animation: "fadeUp 0.42s ease 0.22s both" }}>
            ...
            >
              <NavCard href="/billing/payments"         icon={CreditCard}  title="Payments"         desc="Invoices, history & billing"   colorVar={T.brandMid} bgVar={T.brandLight} delay={60}  />
              <NavCard href="/billing/driver-payouts"   icon={Users}       title="Driver Payouts"   desc="Manage fleet driver payments"  colorVar={T.success}  bgVar={T.successBg}  delay={110} />
              <NavCard href="/billing/awaiting-payment" icon={AlertCircle} title="Awaiting Payment" desc="Outstanding invoices"          badge={pendingCount} colorVar={T.warning} bgVar={T.warningBg} delay={160} />
              <NavCard href="/billing/my-payments"      icon={Wallet}      title="My Payments"      desc="Your personal invoices"        colorVar={T.accent}   bgVar={T.accentBg}   delay={210} />
            </div>
          </section>
      ...
      <ReceiveModal open={activeModal === "receive"} userId={userId} userName="Account" onClose={() => setActiveModal(null)} />
      <SendModal open={activeModal === "send"} onClose={() => setActiveModal(null)} onSuccess={() => {}} getToken={getToken} />
    </>
  );
}