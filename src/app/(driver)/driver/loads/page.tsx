                        ...
                        <div className="flex gap-1 mt-5 bg-white/5 rounded-xl p-1 border border-white/8">
                            {tabItems.map(t => (
                                <button key={t.key} onClick={() => setTab(t.key)}
                                    ...
                                    {t.count && t.count > 0 && (
                                        <span className={cn('inline-flex items-center justify-center min-w-[18px] h-[18px] text-[9px] font-bold rounded-full px-1',
        ...
        <div className="flex items-center gap-0 w-full mt-1">
            {STEPS.map((step, i) => (
                <React.Fragment key={step.key}>
                    ...
                    {i < STEPS.length - 1 && <div className={cn('flex-1 h-0.5 mt-[-10px] mx-0.5 rounded-full', i < cur ? 'bg-emerald-500' : 'bg-border')} />}
                </React.Fragment>
            ))}
        </div>
    );
