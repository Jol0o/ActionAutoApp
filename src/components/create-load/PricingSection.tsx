...
export function PricingSection({ pickupZip, deliveryZip, vehicles }: PricingSectionProps) {
  ...
  return (
    <div className="space-y-3">
      {/* Main rate highlight */}
      <div className="rounded-lg bg-green-500/8 border border-green-500/20 p-4 flex items-center justify-between gap-4">
        <div>
          <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-0.5">Estimated Carrier Pay</p>
          <p className="text-2xl font-bold text-green-600 dark:text-green-400">
            ${result.estimatedRate.toLocaleString()}
          </p>
          <p className="text-[10px] text-muted-foreground mt-0.5">Auto-calculated · editable after posting</p>
        </div>
      ...
      <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
        ...
        Rate via SupraPay — $1.50/mi base, min $300. Adjust carrier pay after posting.
      </div>
    </div>
  )
}
