...
interface BankForm { bank: string; accountNumber: string; accountName: string; amount: string; reference: string; }
interface EpayForm { platform: string; mobileNumber: string; amount: string; reference: string; }
interface UserForm { userId: string; amount: string; notes: string; }
interface QrForm   { recipient: string; amount: string; notes: string; rawData: string; }
type FormErrors = Partial<Record<string, string>>;

// ─── Themed atom components ───────────────────────────────────────────────────

function SupraLabel({ children, htmlFor, required }: {
  ...
  return (
    <label htmlFor={htmlFor} style={{
      ...
      textTransform: "uppercase" as const, color: TEXT_LO, marginBottom: 6, fontFamily: DISPLAY,
    }}>
      {children}
...
function SupraInput({ id, type = "text", placeholder, value, onChange, hasError = false }: {
  ...
  return (
    <input
      ...
      style={{
        ...
        borderRadius: 8, padding: "8px 12px", color: TEXT_HI, outline: "none",
        boxShadow: focused ? `0 0 0 3px rgba(229,90,0,0.13)` : hasError ? "0 0 0 3px rgba(239,68,68,0.10)" : "none",
  ...
  return (
    <textarea
      ...
      style={{
        ...
        borderRadius: 8, padding: "8px 12px", color: TEXT_HI, outline: "none", resize: "none",
...
function FieldError({ msg }: { msg?: string }) {
  if (!msg) return null;
  return <p style={{ fontSize: 11, color: "#F87171", marginTop: 4, fontFamily: DISPLAY }}>{msg}</p>;
}

function SupraBtn({ children, onClick, disabled }: {
...
function BackBtn({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick} style={{
      ...
      <ArrowLeft style={{ width: 14, height: 14, color: "rgba(255,255,255,0.55)" }} />
    </button>
  );
}
...
function ScreenHeader({ title, subtitle, onBack }: {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18, paddingRight: 32 }}>
      {onBack && <BackBtn onClick={onBack} />}
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontSize: 16, fontWeight: 700, color: "#fff", letterSpacing: "-0.01em", lineHeight: 1.1 }}>
          {title}
        </p>
        {subtitle && (
          <p style={{ fontSize: 11, color: TEXT_LO, marginTop: 3, letterSpacing: "0.02em" }}>
            {subtitle}
          </p>
        )}
      </div>
...
export function SendModal({ open, onClose, onSuccess, getToken }: SendModalProps) {
  ...
  const [bankForm, setBankForm] = React.useState<BankForm>({
    bank: "", accountNumber: "", accountName: "", amount: "", reference: "",
  });
  const [epayForm, setEpayForm] = React.useState<EpayForm>({
    platform: "Venmo", mobileNumber: "", amount: "", reference: "",
  });
  const [userForm, setUserForm] = React.useState<UserForm>({
    userId: "", amount: "", notes: "",
  });
  const [qrForm, setQrForm] = React.useState<QrForm>({
    recipient: "", amount: "", notes: "", rawData: "",
  });
  const [qrUploadPreview, setQrUploadPreview] = React.useState<string | null>(null);
  ...
  React.useEffect(() => {
    if (open) {
      setFlow("select"); setSendType(null); setErrors({}); setIsLoading(false);
      setBankForm({ bank: "", accountNumber: "", accountName: "", amount: "", reference: "" });
      setEpayForm({ platform: "Venmo", mobileNumber: "", amount: "", reference: "" });
      setUserForm({ userId: "", amount: "", notes: "" });
      setQrForm({ recipient: "", amount: "", notes: "", rawData: "" });
      setQrUploadPreview(null); setQrDecoding(false); setQrError(null); setCameraError(null);
    } else {
      stopCamera();
    }
  }, [open]);
  ...
  const startScanLoop = () => {
    if (!("BarcodeDetector" in window)) {
      setCameraError("QR scanning is not supported in this browser. Try uploading a QR image instead.");
      stopCamera();
      return;
    }
  ...
  const decodeImageFile = async (file: File) => {
    setQrDecoding(true); setQrError(null);
    try {
      if (!("BarcodeDetector" in window))
        throw new Error("QR decoding not supported in this browser. Enter details manually.");
      const detector = new BarcodeDetector({ formats: ["qr_code"] });
  ...
  const validate = (): boolean => {
    ...
    if (sendType === "bank") {
      ...
      if (!bankForm.accountNumber || bankForm.accountNumber.replace(/\D/g, "").length < 8) e.accountNumber = "Min 8 digits";
      if (!bankForm.accountName.trim()) e.accountName = "Required";
      if (!bankForm.amount || parseFloat(bankForm.amount) <= 0) e.amount = "Invalid amount";
    } else if (sendType === "epay") {
      if (!epayForm.mobileNumber || epayForm.mobileNumber.replace(/\D/g, "").length < 10) e.mobileNumber = "Invalid number";
      if (!epayForm.amount || parseFloat(epayForm.amount) <= 0) e.amount = "Invalid amount";
    } else if (sendType === "user") {
      if (!userForm.userId.trim()) e.userId = "Required";
      if (!userForm.amount || parseFloat(userForm.amount) <= 0) e.amount = "Invalid amount";
    } else if (sendType === "upload-qr" || sendType === "scan-qr") {
      if (!qrForm.recipient.trim()) e.recipient = "Required";
      if (!qrForm.amount || parseFloat(qrForm.amount) <= 0) e.amount = "Invalid amount";
    }
    setErrors(e);
    return Object.keys(e).length === 0;
  };

  const handleProceed = () => { if (validate()) setFlow("confirm"); };
  ...
  const getAmount = () => {
    ...
    if (sendType === "upload-qr" || sendType === "scan-qr") return parseFloat(qrForm.amount || "0");
    return 0;
  };

  const getConfirmRows = (): { label: string; value: string }[] => {
    if (sendType === "bank") return [
    ...
    if (sendType === "user") return [
      { label: "Recipient",      value: userForm.userId },
      { label: "Notes",          value: userForm.notes || "—" },
    ];
    if (sendType === "upload-qr" || sendType === "scan-qr") return [
      { label: "Recipient",      value: qrForm.recipient },
      { label: "Notes",          value: qrForm.notes || "—" },
      { label: "QR data",        value: qrForm.rawData.slice(0, 46) + (qrForm.rawData.length > 46 ? "…" : "") },
    ];
    return [];
  };
  ...
  const getRecipientLabel = () => {
    ...
    if (sendType === "upload-qr" || sendType === "scan-qr") return qrForm.recipient;
    return "";
  };
  ...
  // ── Option card list ────────────────────────────────────────────
  const sendOptions = [
    { key: "bank"      as SendType, Icon: Building2,  label: "Bank transfer",        sub: "Chase, Bank of America, Wells Fargo…",   iconBg: "rgba(59,130,246,0.12)",  iconColor: "#60A5FA" },
    { key: "epay"      as SendType, Icon: Smartphone,  label: "E-payment platform",  sub: "Venmo, Zelle, Cash App, PayPal, Apple Pay", iconBg: "rgba(168,85,247,0.12)", iconColor: "#C084FC" },
    { key: "user"      as SendType, Icon: UserCircle,  label: "System user",          sub: "Transfer within this platform",          iconBg: "rgba(34,197,94,0.11)",   iconColor: "#4ADE80" },
    { key: "upload-qr" as SendType, Icon: ImageUp,     label: "Upload QR code",       sub: "Upload a QR image from your device",     iconBg: "rgba(249,115,22,0.12)",  iconColor: "#FB923C" },
    { key: "scan-qr"   as SendType, Icon: Camera,      label: "Scan QR code",         sub: "Use camera to scan a QR code",           iconBg: "rgba(244,63,94,0.12)",   iconColor: "#FB7185" },
  ];
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ position: "relative", padding: "20px 22px 22px 20px" }}>
            ...
            {flow === "select" && (
              <>
                <ScreenHeader title="Send Money" subtitle="Choose where to send funds" />
                <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                  {sendOptions.map(opt => (
                    <button
                      key={opt.key}
                      onClick={() => { setSendType(opt.key); setFlow(opt.key); }}
                      onMouseEnter={() => setHoveredOpt(opt.key)}
                      onMouseLeave={() => setHoveredOpt(null)}
                      style={{
                        display: "flex", alignItems: "center", gap: 12, padding: "11px 12px",
                        borderRadius: 10, cursor: "pointer", textAlign: "left",
                        background: hoveredOpt === opt.key ? "rgba(255,255,255,0.08)" : "rgba(255,255,255,0.04)",
                    ...
                    >
                      <span style={{
                        ...
                        <opt.Icon style={{ width: 17, height: 17, color: opt.iconColor }} />
                      </span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <p style={{
                          fontSize: 11, color: "rgba(255,255,255,0.30)", marginTop: 2,
                          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                        }}>
                          {opt.sub}
                        </p>
                      </div>
                      <ChevronRight style={{ width: 14, height: 14, color: "rgba(255,255,255,0.22)", flexShrink: 0 }} />
                    </button>
                  ))}
                </div>
              </>
            ...
            {flow === "bank" && (
              <>
                <ScreenHeader
                  title="Bank Transfer" subtitle="Send to any US bank account"
                  onBack={() => { setFlow("select"); setErrors({}); }}
                />
                <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
                  <div>
                    <SupraLabel htmlFor="b-bank" required>Bank name</SupraLabel>
                    <SupraInput id="b-bank" placeholder="Chase, Bank of America…"
                      value={bankForm.bank} onChange={e => setBankForm(f => ({ ...f, bank: e.target.value }))}
                      hasError={!!errors.bank} />
                    <FieldError msg={errors.bank} />
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <div>
                      <SupraLabel htmlFor="b-acct" required>Account number</SupraLabel>
                      <SupraInput id="b-acct" placeholder="Routing + acct number"
                    ...
                    <div>
                      <SupraLabel htmlFor="b-name" required>Account name</SupraLabel>
                      <SupraInput id="b-name" placeholder="Full name"
                    ...
                    <div>
                      <SupraLabel htmlFor="b-amt" required>Amount ($)</SupraLabel>
                      <SupraInput id="b-amt" type="number" placeholder="0.00"
                    ...
                    <div>
                      ...
                      <SupraInput id="b-ref" placeholder="Optional"
                        value={bankForm.reference} onChange={e => setBankForm(f => ({ ...f, reference: e.target.value }))} />
                    </div>
                  </div>
            ...
            {flow === "epay" && (
              <>
                <ScreenHeader
                  title="E-Payment Platform" subtitle="Venmo, Zelle, Cash App and more"
                  onBack={() => { setFlow("select"); setErrors({}); }}
                />
                <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
                  <div>
                    <SupraLabel htmlFor="e-plat" required>Platform</SupraLabel>
                    {/*
                    ...
                    <Select
                      ...
                      onValueChange={v => setEpayForm(f => ({ ...f, platform: v }))}
                    >
                      <SelectTrigger
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <div>
                      <SupraLabel htmlFor="e-num" required>Mobile number</SupraLabel>
                      <SupraInput id="e-num" type="tel" placeholder="(555) 000-0000"
                        value={epayForm.mobileNumber} onChange={e => setEpayForm(f => ({ ...f, mobileNumber: e.target.value }))}
                        hasError={!!errors.mobileNumber} />
                      <FieldError msg={errors.mobileNumber} />
                    </div>
                    <div>
                      <SupraLabel htmlFor="e-amt" required>Amount ($)</SupraLabel>
                      <SupraInput id="e-amt" type="number" placeholder="0.00"
                        value={epayForm.amount} onChange={e => setEpayForm(f => ({ ...f, amount: e.target.value }))}
                        hasError={!!errors.amount} />
                      <FieldError msg={errors.amount} />
                    </div>
                  </div>
                  <div>
                    ...
                    <SupraInput id="e-ref" placeholder="Optional"
                      value={epayForm.reference} onChange={e => setEpayForm(f => ({ ...f, reference: e.target.value }))} />
                  </div>
                  <SupraBtn onClick={handleProceed}>Review transfer</SupraBtn>
                </div>
              </>
            ...
            {flow === "user" && (
              <>
                <ScreenHeader
                  title="Send to User" subtitle="Transfer within the platform"
                  onBack={() => { setFlow("select"); setErrors({}); }}
                />
                <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
                  <div>
                    <SupraLabel htmlFor="u-id" required>User ID or email</SupraLabel>
                    <SupraInput id="u-id" placeholder="USR-XXXX or user@email.com"
                      value={userForm.userId} onChange={e => setUserForm(f => ({ ...f, userId: e.target.value }))}
                      hasError={!!errors.userId} />
                    <FieldError msg={errors.userId} />
                  </div>
                  <div>
                    <SupraLabel htmlFor="u-amt" required>Amount ($)</SupraLabel>
                    <SupraInput id="u-amt" type="number" placeholder="0.00"
                      value={userForm.amount} onChange={e => setUserForm(f => ({ ...f, amount: e.target.value }))}
                      hasError={!!errors.amount} />
                    <FieldError msg={errors.amount} />
                  </div>
                  <div>
                    <SupraLabel htmlFor="u-notes">Notes</SupraLabel>
                    <SupraTextarea id="u-notes" placeholder="Optional message…"
            ...
            {flow === "upload-qr" && (
              <>
                <ScreenHeader
                  title="Upload QR Code" subtitle="Auto-fill recipient from image"
                  onBack={() => { setFlow("select"); setErrors({}); setQrError(null); }}
                />
                <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
                  {/* Drop zone */}
                  <label htmlFor="qr-upload" style={{
                    display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
                    gap: 10, height: 140, borderRadius: 10, cursor: "pointer", transition: "all 0.14s",
                    background: qrUploadPreview ? "rgba(229,90,0,0.06)" : "rgba(255,255,255,0.03)",
                    ...
                    {qrUploadPreview ? (
                      ...
                      <>
                        <span style={{
                          ...
                          <ImageUp style={{ width: 20, height: 20, color: "#FB923C" }} />
                        </span>
                        <div style={{ textAlign: "center" }}>
                          <p style={{ fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.68)" }}>
                  ...
                  {qrDecoding && (
                    <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: TEXT_MID }}>
                      <Loader2 style={{ width: 13, height: 13, animation: "supraSpin 1s linear infinite" }} />
                      Decoding QR code…
                    </div>
                  )}
                  {qrError && (
                    <p style={{ fontSize: 11, color: "#F87171", background: "rgba(239,68,68,0.08)", borderRadius: 8, padding: "8px 12px" }}>
                      {qrError}
                    </p>
                  )}

                  <div>
                    <SupraLabel htmlFor="uqr-rec" required>Recipient</SupraLabel>
                    <SupraInput id="uqr-rec" placeholder="Auto-filled or enter manually"
                      value={qrForm.recipient} onChange={e => setQrForm(f => ({ ...f, recipient: e.target.value }))}
                      hasError={!!errors.recipient} />
                    <FieldError msg={errors.recipient} />
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <div>
                      <SupraLabel htmlFor="uqr-amt" required>Amount ($)</SupraLabel>
                      <SupraInput id="uqr-amt" type="number" placeholder="0.00"
                        value={qrForm.amount} onChange={e => setQrForm(f => ({ ...f, amount: e.target.value }))}
                        hasError={!!errors.amount} />
                      <FieldError msg={errors.amount} />
                    </div>
                    <div>
                      <SupraLabel htmlFor="uqr-notes">Notes</SupraLabel>
                      <SupraInput id="uqr-notes" placeholder="Optional"
                        value={qrForm.notes} onChange={e => setQrForm(f => ({ ...f, notes: e.target.value }))} />
                    </div>
                  </div>
                  <SupraBtn onClick={handleProceed} disabled={qrDecoding}>Review transfer</SupraBtn>
                </div>
              </>
            )}
            ...
            {flow === "scan-qr" && (
              <>
                <ScreenHeader
                  title="Scan QR Code" subtitle="Point camera at a QR code"
                  onBack={() => { stopCamera(); setFlow("select"); setErrors({}); setCameraError(null); }}
                />
                <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
                  {/* Viewfinder / result */}
                  {!qrForm.rawData ? (
                      ...
                      {cameraActive ? (
                            ...
                            <div style={{ width: 172, height: 172, position: "relative" }}>
                              {camCorners.map((s, i) => (
                                <span key={i} style={{
                                  ...
                                  borderColor: ORANGE, borderStyle: "solid", borderWidth: 0, borderRadius: 3, ...s,
                                }} />
                              ))}
                              <div style={{
                                position: "absolute", inset: 0, top: "50%", height: 1,
                                background: `${ORANGE}80`, animation: "supraPulse 1.6s ease-in-out infinite",
                              }} />
                            </div>
                          </div>
                          <button onClick={stopCamera} style={{
                            position: "absolute", top: 8, right: 8, width: 28, height: 28, borderRadius: 20,
                        ...
                        <div style={{
                          ...
                          <span style={{
                            ...
                            <Camera style={{ width: 24, height: 24, color: "#FB7185" }} />
                          </span>
                          <div style={{ textAlign: "center" }}>
                            <p style={{ fontSize: 14, fontWeight: 600, color: "rgba(255,255,255,0.78)" }}>
                              Camera not active
                            </p>
                          ...
                          {!cameraError && (
                            <button onClick={startCamera} style={{
                              ...
                              color: "white", fontSize: 13, fontWeight: 600, fontFamily: DISPLAY, cursor: "pointer",
                            }}>
                    ...
                    /* QR detected state */
                    <div style={{
                      display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "13px 0",
                      ...
                      <p style={{ fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.80)" }}>QR code detected!</p>
                    </div>
                  )}
                  ...
                  <div>
                    <SupraLabel htmlFor="sqr-rec" required>Recipient</SupraLabel>
                    <SupraInput id="sqr-rec" placeholder="Auto-filled after scan"
                      value={qrForm.recipient} onChange={e => setQrForm(f => ({ ...f, recipient: e.target.value }))}
                      hasError={!!errors.recipient} />
                    <FieldError msg={errors.recipient} />
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <div>
                      <SupraLabel htmlFor="sqr-amt" required>Amount ($)</SupraLabel>
                      <SupraInput id="sqr-amt" type="number" placeholder="0.00"
                        value={qrForm.amount} onChange={e => setQrForm(f => ({ ...f, amount: e.target.value }))}
                        hasError={!!errors.amount} />
                      <FieldError msg={errors.amount} />
                    </div>
                    <div>
                      ...
                      <SupraInput id="sqr-notes" placeholder="Optional"
                        value={qrForm.notes} onChange={e => setQrForm(f => ({ ...f, notes: e.target.value }))} />
                    </div>
                  </div>
                  <SupraBtn onClick={handleProceed}>Review transfer</SupraBtn>
            ...
            {flow === "confirm" && (
              <>
                <ScreenHeader
                  ...
                  onBack={() => { setFlow(sendType!); }}
                />
                <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
                  {/* Amount highlight */}
                  <div style={{
                    background: "rgba(229,90,0,0.09)", border: "1px solid rgba(229,90,0,0.22)",
                    borderRadius: 12, padding: "18px 20px", textAlign: "center",
                  }}>
                    <p style={{
                      fontSize: 10, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase" as const,
                      color: "rgba(229,90,0,0.68)", marginBottom: 6, fontFamily: DISPLAY,
                    }}>
                      You are sending
                    </p>
                    <p style={{ fontSize: 40, fontWeight: 700, color: ORANGE, letterSpacing: "-0.02em", fontFamily: DISPLAY }}>
                      {fmtAmt(getAmount())}
                    </p>
                  </div>
                    ...
                    {getConfirmRows().map(({ label, value }, i) => (
                      <div key={label} style={{
                        ...
                        <span style={{
                          ...
                          textTransform: "uppercase" as const, color: "rgba(255,255,255,0.32)", fontFamily: DISPLAY,
                        }}>
                          {label}
                        </span>
                        <span style={{
                          fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.80)",
                          maxWidth: "58%", textAlign: "right", overflow: "hidden",
                  ...
                  <SupraBtn onClick={handleConfirm} disabled={isLoading}>
                    {isLoading ? (
                      <>
                        <Loader2 style={{ width: 14, height: 14, animation: "supraSpin 1s linear infinite" }} />
                        Processing…
                      </>
                    ) : (
                      <>
            ...
            {flow === "success" && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", padding: "16px 0 8px" }}>
                <div style={{
                  ...
                  background: "rgba(229,90,0,0.12)", border: "1px solid rgba(229,90,0,0.25)",
                ...
                <p style={{ fontSize: 21, fontWeight: 700, color: "#fff", letterSpacing: "-0.01em", marginBottom: 6 }}>
                  Transfer successful
                </p>
                <p style={{ fontSize: 13, color: "rgba(255,255,255,0.38)", marginBottom: 22, fontFamily: DISPLAY }}>
