    ...
    <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-slate-50/50 dark:bg-background">
      ...
      <div className="flex items-start justify-between gap-4 px-6 py-6 border-b border-border/40 bg-background shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-lg font-bold text-white truncate">{lead.firstName} {lead.lastName}</h2>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg text-slate-700 hover:text-slate-300 hover:bg-[#1e3327] transition-colors">
            <X className="h-4 w-4" />
          </button>
        </div>
      </div>
      ...
      <div className="px-6 py-3.5 border-b border-border/40 bg-card shrink-0">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
          <div className="flex items-center gap-1.5 text-[11px]">
            ...
            <span className="text-slate-300 font-medium text-[12px]">{lead.subject || '(No subject)'}</span>
          </div>
          ...
          <span className="text-[10px] font-mono text-slate-700 ml-auto">via {sourceEmail}</span>
        </div>
        {lead.appointment && (
      ...
      >
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 max-w-2xl">
            <div className="flex items-baseline gap-2 mb-2">
              <span className="text-[14px] font-bold text-white">{lead.firstName} {lead.lastName}</span>
              <span className="text-xs text-slate-600">{fmtFull(new Date(lead.createdAt))}</span>
            </div>
            <div className="rounded-2xl rounded-tl-sm border border-border/40 bg-card px-7 py-6 shadow-2xl shadow-black/80 ring-1 ring-inset ring-white/[0.03]">
              <ParsedContent content={lead.parsedContent} rawBody={lead.body} />
            </div>
          </div>
        </div>
        ...
        {threads.length > 0 && (
          <div className="flex items-center gap-3">
            ...
            <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-emerald-900">{threads.length} repl{threads.length === 1 ? 'y' : 'ies'}</span>
