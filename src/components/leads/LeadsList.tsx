...
export const LeadsList = React.memo(
  ({
    ...
    return (
      <div
        className={`flex flex-col bg-card border-r border-border shrink-0 w-[320px] xl:w-[360px] shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-10 ${selectedLeadId ? "hidden lg:flex" : "flex"}`}
      >
        {/* Search bar */}
        ...
        <div
          ref={listRef}
          className="flex-1 overflow-y-auto min-h-0 space-y-2 scrollbar-dark"
        >
          {isLoading ? (
            <div className="flex flex-col p-4 space-y-3">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div
                  key={i}
                  className="h-20 w-full rounded-2xl bg-muted/40 animate-pulse border border-border/40"
                />
              ))}
            </div>
          ) : leads.length === 0 ? (
              ...
              return (
                <div
                  ...
                  className={`group relative overflow-hidden p-3 rounded-2xl border cursor-pointer transition-all duration-300 mx-2 ${
                    sel
                      ? "border-emerald-500/50 bg-emerald-500/10 shadow-md ring-2 ring-emerald-500/10 scale-[1.01] z-10"
                      : isHighlighted
                        ? "border-emerald-500 bg-emerald-500/20 shadow-lg shadow-emerald-500/20 ring-2 ring-emerald-500 animate-pulse"
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-0.5">
                        <p
                          className={`text-sm font-semibold truncate leading-tight ${lead.isRead ? "text-muted-foreground/70" : "text-foreground"}`}
                        >
                      ...
                      <p
                        className={`text-xs truncate leading-snug ${lead.isRead ? "text-muted-foreground/50" : "text-muted-foreground"}`}
                      >
                  ...
                  {sel && (
                    <div className="absolute inset-y-0 left-0 w-0.5 bg-emerald-400 rounded-r" />
                  )}
                </div>
              );
            })
