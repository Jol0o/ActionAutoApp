...
// ─── Badge CSS — scoped to [data-sl] attribute ────────────────────────────────
const BADGE_CSS = `
...
/* Light mode defaults (explicit — not relying on :root inheritance) */
[data-sl] {
  --slb-g:    #b8892e;
  --slb-g2:   #d4a84b;
  --slb-gd:   rgba(184,137,46,0.09);
  --slb-gb:   rgba(184,137,46,0.18);
  --slb-surf: #ffffff;
  --slb-bg2:  #f6f4f0;
  --slb-bd:   rgba(184,137,46,0.15);
  --slb-bd2:  rgba(0,0,0,0.07);
  --slb-tx:   #1a1610;
  --slb-tx2:  #574e3e;
  --slb-tx3:  rgba(80,68,50,0.46);
  --slb-rad:  3px;
...
@media (prefers-color-scheme: dark) {
  [data-sl] {
    --slb-surf: #17140e;
    --slb-bg2:  #1e1a13;
    --slb-bd:   rgba(184,137,46,0.2);
    --slb-bd2:  rgba(255,255,255,0.06);
    --slb-tx:   rgba(250,244,232,0.92);
    --slb-tx2:  rgba(218,196,162,0.64);
    --slb-tx3:  rgba(218,196,162,0.32);
...
[data-sl] .slb-name {
  ...
  color:var(--slb-g2); letter-spacing:.08em; line-height:1; display:block; margin-bottom:3px;
}
[data-sl] .slb-status {
  display:flex; align-items:center; gap:4px;
  font-family:'JetBrains Mono',monospace; font-size:7.5px;
  letter-spacing:.17em; text-transform:uppercase; color:var(--slb-tx3);
}
...
function injectBadgeCSS() {
  ...
  if (document.getElementById('sl-badge-v5')) return
...
// ─── Toolbar Badge ────────────────────────────────────────────────────────────
function LeoToolbarBadge({ state, onClick }: { state: LeoState; onClick: () => void }) {
  const d = DOTS[state]
  return (
    <div data-sl>
...
export function SupraLeoAI({
  ...
  // Lazy-load Panel to avoid SSR issues with CSS injection
  useEffect(() => {
  ...
  // Listen for read-message events from SupraLeoReadButton
  useEffect(() => {
    const h = (e: Event) => {
  ...
  return (
    ...
    >
      ...
      <LeoBadge state={state} onClick={() => setOpen(p => !p)} />
    </div>
  )
}
