...
const ShipmentCardInner = React.memo(function ShipmentCardInner({
  ...
  const getStatusConfig = () => {
    switch (shipment.status) {
      ...
      default:
        return {
          color: "bg-linear-to-r from-gray-500 to-slate-500",
            ...
            {quote?.vehicleImage ? (
              <div className="relative w-full h-40 sm:h-56 md:h-64">
                ...
                <div className="absolute top-2 left-2 right-2 flex items-start justify-between flex-wrap gap-1.5">
                  ...
                  <Badge
                    className={`${statusConfig.color} text-white px-2 py-0.5 text-[10px] sm:text-xs font-semibold shadow-lg ml-auto`}
                  >
                ...
                <div className="bg-linear-to-br from-blue-50 to-indigo-50 dark:from-blue-950 dark:to-indigo-950 rounded-lg p-3 border border-blue-200 dark:border-blue-800 shadow-sm">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {quote?.vehicleInoperable && (
                      <Badge
                        variant="outline"
                        className="bg-orange-50 dark:bg-orange-950 text-orange-700 dark:text-orange-400 border-orange-300 dark:border-orange-700 text-xs font-semibold"
                      >
                        Inoperable
                      </Badge>
                    )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                ...
                <div className="bg-linear-to-br from-blue-50 via-cyan-50 to-teal-50 dark:from-blue-950 dark:via-cyan-950 dark:to-teal-950 rounded-lg p-3 sm:p-4 border border-blue-200 dark:border-blue-800 shadow-sm h-full">
                  <div className="flex items-center gap-2 mb-3 sm:mb-4">
                    ...
                    <h3 className="text-[10px] sm:text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">
                      Shipping Route
                    </h3>
                  </div>
                    ...
                    <div className="group mb-2 sm:mb-4">
                      <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-lg p-2.5 sm:p-4 border-2 border-green-300 dark:border-green-700 shadow-md transition-all">
                        <div className="flex items-center gap-2 sm:gap-3 mb-1 sm:mb-2">
                          ...
                          <span className="text-[8px] sm:text-xs font-bold text-green-600 dark:text-green-400 uppercase tracking-wide block leading-none">
                            Starting Point
                          </span>
                        </div>
                        <p className="font-bold text-gray-900 dark:text-gray-100 text-xs sm:text-base leading-relaxed pl-1 truncate">
                          {shipment.origin}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="group mt-2 sm:mt-4">
                      <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-lg p-2.5 sm:p-4 border-2 border-red-300 dark:border-red-700 shadow-md transition-all">
                        <div className="flex items-center gap-2 sm:gap-3 mb-1 sm:mb-2">
                          ...
                          <span className="text-[8px] sm:text-xs font-bold text-red-600 dark:text-red-400 uppercase tracking-wide block leading-none">
                            Final Stop
                          </span>
                        </div>
                        <p className="font-bold text-gray-900 dark:text-gray-100 text-xs sm:text-base leading-relaxed pl-1 truncate">
                      ...
                      {[
                        ...
                        >
                          <div className="relative z-10 shrink-0">
                            <div
                              className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-linear-to-br from-${item.color}-500 to-${item.color}-600 flex items-center justify-center shadow-md ring-2 sm:ring-4 ring-${item.color}-100 dark:ring-${item.color}-900 group-hover:ring-${item.color}-200 dark:group-hover:ring-${item.color}-800 transition-all`}
                            >
                              {item.icon ? (
                          ...
                          <div
                            className={`flex-1 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg p-2 sm:p-3 border border-${item.color}-200 dark:border-${item.color}-700 shadow-sm group-hover:shadow-md transition-all flex items-center justify-between`}
                          >
                            <div className="min-w-0">
                              <p
                                className={`text-[10px] sm:text-sm font-bold text-${item.color}-700 dark:text-${item.color}-400 leading-none mb-0.5 truncate`}
                              >
                                {item.label}
                              </p>
                              <p className="text-[9px] sm:text-xs font-semibold text-gray-900 dark:text-gray-100 leading-none">
                                {formatDate(item.date)}
                              </p>
                            </div>
                            <span
                              className={`text-[8px] sm:text-xs px-1.5 sm:px-2 py-0.5 bg-${item.color}-100 dark:bg-${item.color}-900 text-${item.color}-700 dark:text-${item.color}-300 rounded-full font-semibold shrink-0`}
                            >
                              {item.step}
                            </span>
                          </div>
...
export function ShipmentCard({
  ...
  const handleSaveEdit = React.useCallback(
    async (updatedShipment: Partial<Shipment>) => {
      if (!onUpdate) {
  ...
  const handleSendEmail = React.useCallback(() => {
    ...
    if (!customerEmail || !isValidEmail(customerEmail)) {
      showAlert({
        type: "error",
        title: "Invalid Email",
        message: "This shipment does not have a valid recipient email.",
      });
      return;
    }
      ...
      onConfirm: async () => {
        try {
          await apiClient.post(
            `/api/shipments/${shipment._id}/send-details-email`,
            {
