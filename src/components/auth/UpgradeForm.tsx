...
import { Loader2, Mail, Lock, ShieldCheck, ArrowRight, AlertCircle, CheckCircle2 } from "lucide-react";
...
export function UpgradeForm() {
    ...
    const [step, setStep] = useState<'request' | 'verify'>(emailFromUrl ? 'verify' : 'request');
    ...
    const handleUpgrade = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err.response?.data?.message || "Upgrade failed. Check your code or try again.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        ...
        >
            <Card className="border-border/50 bg-card/50 backdrop-blur-xl shadow-2xl overflow-hidden">
                ...
                <CardHeader>
                    <CardTitle className="text-2xl font-bold text-center">Secure Your Account</CardTitle>
                    ...
                    <AnimatePresence mode="wait">
                        {step === 'request' ? (
                            ...
                            >
                                ...
                                {error && <div className="text-xs text-destructive flex items-center gap-1"><AlertCircle className="h-3 w-3" /> {error}</div>}
                            ...
                            >
                                ...
                                <Button type="submit" className="w-full h-11" disabled={isLoading}>
                                    {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Upgrade & Complete"}
                                </Button>
